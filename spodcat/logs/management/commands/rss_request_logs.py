...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        for path_info, logs in itertools.groupby(
            PodcastRssRequestLog.objects.using(options["database"]).order_by("path_info", "created"),
            ...
            referrers = {r: [log for log in logs if log.referrer == r] for r in set(log.referrer for log in logs) if r}
            ...
            if referrers:
                ...
                for ref, ref_logs in referrers.items():
                    ...
                    self.stdout.write(f" * {ref}: {len(ref_logs)} / {ref_percent:.02f}% ({len(ref_uq_ips)} unique IPs)")
