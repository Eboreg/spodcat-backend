...
class CommentSerializerMixin:
    def get_podcast_content(self, attrs: dict) -> PodcastContent:
        return attrs["podcast_content"]

    def get_text_html(self, obj: Comment) -> str:
        return obj.text_html

    @staticmethod
    def send_email(podcast_content: PodcastContent, to: str, commenter: str):
        if not podcast_content.podcast.require_comment_approval:
            email_text = _("A new comment for %(podcast)s was just posted by %(name)s. Check it out here: %(url)s") % {
