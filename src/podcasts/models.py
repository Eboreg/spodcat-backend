...
class PodcastLink(models.Model):
    ...
    link_type = models.CharField(max_length=10, choices=Type)
    url = models.URLField()
...
class Podcast(models.Model):
    ...
    banner = models.ImageField(null=True, default=None, blank=True, upload_to=podcast_image_path)
...
class Episode(models.Model):
    name = models.CharField(max_length=100)
    description = MDTextField(null=True, default=None, blank=True)
    episode = models.PositiveSmallIntegerField(null=True, default=None, blank=True)
    audio_file = models.FileField(upload_to=episode_audio_file_path)
    podcast: "Podcast" = models.ForeignKey("Podcast", on_delete=models.PROTECT, related_name="episodes")
    ...
    class Meta:
        ordering = ["-episode", "-published"]

    @property
    def frontend_url(self):
        # pylint: disable=no-member
        return urljoin(settings.FRONTEND_ROOT_URL, f"{self.podcast.slug}/{self.slug}")
    ...
    def generate_slug(self):
        ...
        if self.episode is not None:
            base_slug = f"{self.episode}-" + base_slug
